...
data-one-select - селекты в средине оболочки с атрибутом показываються только по одному
data-pseudo-label - добавляет псевдоэлемент к заголовку селекта с обозначенным текстом
...
class SelectConstructor {
	constructor(props, data = null) {
		...
		// CSS класы модуля
		this.selectClasses = {
			classSelect: "select", // Головной блок
			...
			classSelectLink: "select__link", // Ссылки на елементы
			...
			classSelectData: "select__asset", // Дополнительный данные
			...
			classSelectMultiple: "_select-multiple", // Мультивибор
			classSelectCheckBox: "_select-checkbox", // Стиль чекбокса
			classSelectOptionSelected: "_select-selected", // Вибраный пункт
			classSelectPseudoLabel: "_select-pseudo-label", // Псевдолейбл
		}
		this._this = this;
		// Запуск инициализацыи
		if (this.config.init) {
			// Получение всех select на строинце
	...
	// Конструктор CSS класа
	getSelectClass(className) {
		return `.${className}`;
	}
	// Геттер елементов псевдоселекта
	...
	// Функция инициализацыи всех селектов
...
// Запускаєем и добавляем в объект модули  
